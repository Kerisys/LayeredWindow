...
public:
	...
	bool AxisX = true;		// 왼쪽/오른쪽
	bool IsJumping = false;	// 점프중인가
	bool IsBreath = false;  // 브레스 스킬 시전중인가
	...
	void Update() {
		...
		if (IsBreath) {
			if (index < breath.size()) {
				std::chrono::duration<double> deltaTime = (std::chrono::system_clock::now() - lastUpdate);
				if (0.1f <= deltaTime.count()) {
